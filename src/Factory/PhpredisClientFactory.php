...
class PhpredisClientFactory
{
     ...
     * @param array{sentinel: ?string} $options
     */
    private function createClientFromSentinel(array $dsns, string $alias, array $options, bool $loggingEnabled): Redis
    {
        foreach ($dsns as $dsn) {
            $address = (new RedisSentinel($dsn->getHost(), (int) $dsn->getPort()))->getMasterAddrByName($options['sentinel']);
        ...
        throw new InvalidArgumentException(
            sprintf(
                ...
                var_export($options['sentinel'], true),
